...
module.exports = function(req, post, isOAuth, callback) {
	...
	if (!error.length) {
		if(post.oauth) {
			...
			collection.findOne({ $or: [{email: post.email}, {login: {$regex: new RegExp('^' + post.login + '$', 'i')}}]}, function (err, result) {
				if (err) throw err;
				const userResult = result;
				if (result !== null) {
					if (!error.includes('email') && post.email === result.email) {
						error.push('email');
					}
					else if (!error.includes('login') && post.login.toLowerCase() === result.login.toLowerCase()) {
						error.push('login');
					}
					callback(error, 1);
				}
